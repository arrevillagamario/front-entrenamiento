...
const Ejercicios = () => {
  ...
  useEffect(() => {
    const fetchEjercicios = async () => {
      try {
        ...
        setEjercicios(response.data.ejercicios); // Asegúrate de que `response.data.ejercicios` es el formato correcto.
        setLoading(false);
      } catch (error) {
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-screen bg-gray-900">
        {/* Loader SVG */}
        <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="white" className="animate-spin">
          <circle cx="12" cy="2" r="0" fill="currentColor">
            <animate attributeName="r" begin="0" calcMode="spline" dur="1s" keySplines="0.2 0.2 0.4 0.8" repeatCount="indefinite" values="0;2;0;0" />
          </circle>
          <circle cx="12" cy="2" r="0" fill="currentColor" transform="rotate(45 12 12)">
            <animate attributeName="r" begin="0.125s" calcMode="spline" dur="1s" keySplines="0.2 0.2 0.4 0.8" repeatCount="indefinite" values="0;2;0;0" />
          </circle>
        </svg>
      </div>
    );
  ...
  return (
    <div className="w-full flex justify-center items-center h-screen bg-gray-900 p-4">
